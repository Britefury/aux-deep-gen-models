...
class SDGMSSL(Model):
    ...
    def build_model(self, train_set_unlabeled, train_set_labeled, test_set, validation_set=None):
        ...
        t_u = t_eye.reshape((1, self.n_y, self.n_y)).repeat(bs_u, axis=0).reshape((-1, self.n_y))
        x_u = self.sym_x_u.reshape((bs_u, 1) + self.shape_x).repeat(self.n_y, axis=1).reshape((-1,) + self.shape_x)
